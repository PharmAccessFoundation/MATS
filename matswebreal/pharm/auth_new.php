...
if (isset($_POST['signup'])) {
    ...
    $password = mysqli_real_escape_string($conn, htmlspecialchars(md5(trim($_POST['password']))));
...
if (isset($_POST['earnload'])) {
    ...
    $end =  $rsplit[1];
     ...
     $rrr = mysqli_query($conn, "SELECT * FROM default_payments_markups where account_user_id = $regid and txn_date BETWEEN '$from' AND  '$end'");

    $login = mysqli_num_rows($rrr);
    $dp = '';
    //echo $login;
        $dp = '<table class="table table-striped tx-table"><thead><tr><td>Date</td><td>Event Triggered</td><td>Amount</td></tr></thead> <tbody>';
    if ($login != 0) {

        foreach ($rrr as $v) {
            
            $dp .= '<tr><td>'.date_format(date_create($v['txn_date']),"Y-m-d").'</td><td>'.ucfirst($v['txn_type']).' Case</td><td>N'.$v['amount'].'</td></tr>';
        }
       
    } else {
        $dp .= '<tr><td colspan="3">No Earning For This Account</td></tr>';
    }
    $dp .= '</tbody></table>';
    echo $dp;
}
...
if (isset($_POST['txload'])) {
    ...
    $end =  $rsplit[1];
     ...
     $rrr = mysqli_query($conn, "SELECT rd.date_added, rd.amount, rd.response_reason as resp FROM default_payments_records rd JOIN default_payments_accounts a on a.id = rd.account_id JOIN default_payments_requests r on r.schedule_id = rd.schedule_id where a.user_id = $regid and r.phase = 'process' and rd.date_added BETWEEN '$from' AND  '$end'");

    $login = mysqli_num_rows($rrr);
    $dp = '';
    //echo $login;
        $dp = '<table class="table table-striped tx-table"><thead><tr><td>Date</td><td>Transaction Status</td><td>Amount</td></tr></thead> <tbody>';
    if ($login != 0) {

        foreach ($rrr as $v) {
            
            $dp .= '<tr><td>'.date_format(date_create($v['date_added']),"Y-m-d").'</td><td>'.ucfirst($v['resp']).'</td><td>N'.$v['amount'].'</td></tr>';
        }
       
    } else {
        $dp .= '<tr><td colspan="3">No Transaction Log For This Account</td></tr>';
    }
    $dp .= '</tbody></table>';
    echo $dp;
}
...
if (isset($_POST['login'])) {
    $email = mysqli_real_escape_string($conn, htmlspecialchars(trim($_POST['email'])));
    $password = mysqli_real_escape_string($conn, md5(htmlspecialchars(trim($_POST['password']))));
...
if (isset($_POST['change_password'])) {
    $email = mysqli_real_escape_string($conn, htmlspecialchars(trim($_POST['email'])));
    $old_password = mysqli_real_escape_string($conn, htmlspecialchars(md5(trim($_POST['old_password']))));
    $new_password = mysqli_real_escape_string($conn, htmlspecialchars(md5(trim($_POST['new_password']))));
