$(function(){pyro.filter={$content:$('#filter-stage'),$filter_form:$('#filters form'),f_module:$('input[name="f_module"]').val(),init:function(){$('a.cancel').button();$('select',pyro.filter.$filter_form).on('change',function(){form_data=pyro.filter.$filter_form.serialize();pyro.filter.do_filter(pyro.filter.f_module,form_data);});$('input[type="text"]',pyro.filter.$filter_form).on('input',$.debounce(500,function(){form_data=pyro.filter.$filter_form.serialize();pyro.filter.do_filter(pyro.filter.f_module,form_data);}));$('body').on('click','.pagination a',function(e){e.preventDefault();url=$(this).attr('href');form_data=pyro.filter.$filter_form.serialize();pyro.filter.do_filter(pyro.filter.f_module,form_data,url);});$('a.cancel',pyro.filter.$filter_form).click(function(){$('select',pyro.filter.$filter_form).val('0');$('input[type="text"]').val('');form_data=pyro.filter.$filter_form.serialize();pyro.filter.do_filter(pyro.filter.f_module,form_data);});pyro.filter.$filter_form.submit(function(e){e.preventDefault();});pyro.filter.$filter_form.find('select').first().trigger('change');},do_filter:function(module,form_data,url){form_action=pyro.filter.$filter_form.attr('action');post_url=form_action?form_action:SITE_URL+'admin/'+module;if(typeof url!=='undefined'){post_url=url;}
