...
//$conn = mysqli_connect("thebuston.com.ng", "thebusto_main", "busayo123$", "thebusto_pharm") or die("could not connect server");
...
if (isset($_POST['supost'])) {
    ...
    if ($login != 0) {
        $data = mysqli_fetch_array($rrr);
         //$q = mysqli_query($conn, "update `default_phonegap_surveys` set `respondent`=$respondent,`cough`=$cough,`more`=$more,`growth`=$growth,`details`='$details',`facility_id`=$facility,`status`='$status',`fever`='$fever',`nightsweat`='$nightsweat',`weightloss`='$weightloss',`date_screened`='$datescreened' where `firstname`='$name' and `mobile`='$mobile'");
        echo "exist:" . $data['status'];
    } else {
        //$q = mysqli_query($conn, "insert into `default_phonegap_surveys` (`firstname`,`mobile`,`respondent`,`cough`,`more`,`growth`,`details`,`facility_id`,`status`,`fever`,`nightsweat`,`weightloss`,`date_screened`) values ('$name','$mobile',$respondent,$cough,$more,$growth,'$details',$facility,'$status','$fever','$nightsweat','$weightloss','$datescreened')");
        //echo $q;
        //if ($q) {
        if (true) {
            echo "success:0";
