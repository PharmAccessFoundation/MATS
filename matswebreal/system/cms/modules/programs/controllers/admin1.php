...
//get all the states available to a programme
        $states = $this->sub_recipient_m
                ->select('*')
                ->where(array('id' => $idd))
                ->get_all();
       //get all the programme the current user manages 
        $progid = $this->getAllProgID();
        //get the list of SR available for the programmes
        $lists = $this->user_m
                ->join('profiles p', 'p.user_id = users.id')
                ->select('*')
                ->where(array('users.group_id' => 9))
                ->where_in('p.manager', $progid)
                ->get_all();
        
        $srlist = $this->my_sr_dropdown('srlist', $lists);

        if ($_POST) {
            $state = $this->input->post('state');
            $sruser = $this->input->post('srlist');
            $uidd = $idd;
            
            if(trim($state) == '' || trim($sruser) == '' ){
                $er = 'All field compulsory';
           ...
           /* var_dump($st);
            ...
            echo $uidd;
            echo '<br/>';*/
           // var_dump(array_search($state, array_map('trim', $st)));
            if((array_search(trim($state),  array_map('trim', $st), TRUE) == FALSE)){
            $up1 = $this->db
                    ->where('id', (int)$idd)
                    ->set('coverage', $states[0]->coverage.', '.$state)
                    ->update('sub_recipients');
    ...
    private function my_sr_dropdown($name, $result_array) {
       ...
       $selecty .=  '<option value="" selected="selected" >- Select -</option>';
       foreach ($result_array as $key => $value) {
          $selecty .= '<option value="'.trim($value->user_id).'">'.$value->display_name.'</option>';
       }
        $selecty .= '</select>';
        return $selecty;
        //return form_dropdown($name, $options, '', "id=$name");
    ...
    public function facility() {
        ...
        if ($this->current_user->group_id == 9) {
         ...
         if((int)$this->current_user->group_id == 7) {
                    $sql = '*, facility_details.id as iddd, p.first_name as ppfirst, p.last_name as pplast, facility_details.state as statt';
                    }else{
                    $sql = '*, facility_details.id as iddd, p.first_name as first, p.last_name as last, facility_details.state as statt';
                    }
