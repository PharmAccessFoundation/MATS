...
class Admin extends Admin_Controller {
    ...
    public function createfinance(){
        if ($this->current_user->group_id != 14 && $this->current_user->group_id != 15) {
            redirect('admin/programs');
        }
// Extra validation for basic data
       ...
       // $mobile = $this->input->post('mobile');
        ...
        $_POST = array_merge(array('mobile' => $phone), $_POST);
...
if ($this->form_validation->run() !== false) {
            ...
            if ($user_id = $this->ion_auth->register($username, $password, $email, $group_id, $profile_data, $group->name)) {
                ...
                $sub = "MATS State Financial Administrator Account Creation";
                $name = $this->input->post('first_name') . ' ' . $this->input->post('last_name');
                $msg = "Dear $name, " . "\r\n\r\n" . "Your account has been created by a MATS Financial administrator." . "\r\n"
                ...
                $upstate = $this->db
                        //->where(array('name' => $this->input->post('state').' State'))
                        ->like('name', $this->input->post('state'))
                        ->set('state_fin_admin_id', $user_id)
                        ->update('states');

                // Redirect back to the form or main page
                $this->input->post('btnAction') === 'save_exit' ? redirect('admin/programs/statefinance') : redirect('admin/programs/statefinance');
            }
            // Error
            else {
        ...
        $this->template
                ->title('Create Financial Administrator')
                ...
                ->build('admin/createfinance');
    }

    public function statefinance() {
         if ($this->current_user->group_id != 14 && $this->current_user->group_id != 15) {
            redirect('admin/programs');
        }
/*
        if ($this->current_user->group_id == 9) {
            $users = $this->user_m
                    ...
                    ->join('profiles p', 'p.user_id = users.id')
                    //->join('states s', '')
                    ->where(array('users.group_id' => 4, 'p.created_by' => $this->current_user->id))
        ...
        $users = $this->user_m
                    ->select('*')
                    ->join('profiles p', 'p.user_id = users.id')
                    ->where(array('users.group_id' => 14))
                    ->order_by('p.created', 'desc')
                    ->get_all();

        $this->template
                ->title($this->module_details['name'].' - Manage State Financial Coordinator')
                //->set('err', $err)
               ->set('users', $users)
                //->set('cusers', $cusers)
                ->build('admin/statefinance');
    }
...
// Extra validation for basic data
    ...
    public function subrep() {
        ...
        $users = $this->user_m
                ->select('*')
                ->join('profiles p', 'p.user_id = users.id')
                ->where(array('users.group_id' => 9, 'p.manager' => $this->current_user->id ))
                ->order_by('p.created', 'desc')
                ->get_all();
...
//$username, $password, $email, $group_id
    ...
    public function managefield($idd) {
        ...
        $field_id = $idd;
        $manager_id = 8; //$this->current_user->id;
        $prog = $this->getProg();
        $progid = 8; //$prog[0]->id;
        $confacs = array();
    ...
    public function createnew() {
        ...
        if ($_POST) {
            ...
            $pid = 8; //$this->input->post('prog');
            $address = $this->input->post('address');

            // var_dump($_POST); exit;

            if (trim($code) == '' || trim($name) == '' || trim($state) == '' || trim($lga) == '' || trim($numb) == '' || trim($address) == '') {
            ...
            $in = $this->facility_detail_m->insert(array('name' => $name, 'address' => $address, 'code' => $code, 'email' => $code, 'state' => $state, 'phone' => $numb, 'program_id' => $pid, 'lga' => $lga));
            if ($in) {
    ...
    public function getscreened() {
        ...
        //   $base_where = $this->input->post('prog') ? $base_where + array('phonegap_surveys.program_id' => (int) $this->input->post('prog')) : $base_where;
        //}

        $progid = 8; // $this->getAllProgID();
        // Keyphrase param
        ...
        if ($this->current_user->group_id == 9) {
            ...
            $users = $this->phonegap_survey_m
                    ...
                    ->where(array('fa.manager_user_id' => $manager_id))
...
public function registered($fid = 0) {
    ...
    public function fdapprove($uid) {
        ...
        if ($up) {
            ...
            $msg = "Dear $name, " . "\r\n" . "" . "\r\n" . "Your account has been deactivated by your MATS Programme Admin. " . "\r\n" . "If you require any clarification please contact Dr. Egbedeyi Olufunsho via f.egbedeyi@pharmaccess.org. " . "\r\n\r\n" . "Best Regards," . "\r\n" . "MATS Admin.";
            //exit;
            $this->sendMail($email, $sub, $msg);
    ...
    public function reassign($id = 0) {
        ...
        $drsta = '<select name="statee" id="state" class="form-control">
                                <option value="" selected="selected" >- Select -</option>';

        foreach ($states as $sta) {
            $drsta .= "<option value='" . trim($sta->name) . "'>" . trim($sta->name) . "</option>";
        }
        $drsta .= '</select>';
