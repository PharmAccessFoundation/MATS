...
//echo $this->current_user->sub_recipient_type; exit;
    ...
    public function registered($fid = 0) {
        ...
        if ($this->input->post('state') && $this->input->post('state') != '') {
            $base_where = $this->input->post('state') ? $base_where + array('f.state' => $this->input->post('state')) : $base_where;
        }
        if ($this->input->post('srlist') && $this->input->post('srlist') != '') {
            $base_where = $this->input->post('srlist') ? $base_where + array('f.sr_type' => (int) $this->input->post('srlist')) : $base_where;
        }
        // Keyphrase param
...
if ($this->current_user->group_id == 9) {
    ...
    public function reassign($id = 0) {
        ...
        if ($this->current_user->group_id == 9 || $this->current_user->group_id == 7) {
            $field_id = $this->current_user->id;
            $manager_id = 8;
