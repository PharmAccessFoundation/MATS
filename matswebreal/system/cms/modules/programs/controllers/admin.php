...
class Admin extends Admin_Controller
{
                    ...
                    $in = $this->state_m
                        ->insert(
                            array(
                                'name' => $my->name,
                                'sr_id' => $my->sr_id,
                                'state_sr_id' => $my->state_sr_id,
                                'zone_id' => $my->zone_id,
                                'cord_id' => $user_id,
                                'state_fin_admin_id' => $my->state_fin_admin_id
                            )
                        );
                    if ($in) {
                        $this->session->set_flashdata('success', 'New State Administrator Have Been Created');
                        $er = 'New State Administrator Have Been Created';
                        redirect('admin/programs/location');
                    }
                } else {
                    $upstate = $this->db
    ...
    public function createfield()
    {
        ...
        $progr = $this->getSRProgramName($this->current_user->sub_recipient_type);
        $subr = $this->getSRName($this->current_user->sub_recipient_type);
        //var_dump($subr); var_dump($progr); exit;
        $this->template
    ...
    public function getscreened()
    {
        ...
        if ((int) $pagination['offset'] != 0) { //Still on pagination
            $base_where = $_SESSION['basewhr'];
            //  echo 'limit != 000';
        } else {
            $_SESSION['basewhr'] = $base_where;
            //  echo 'limit = 0';
        }
        // var_dump($base_where);

        if ($this->current_user->group_id == 9) {
            if ($this->getSRManager($this->current_user->sub_recipient_type) == $this->current_user->email) { //overal SR Manager
                $manager_id = 8;
                ...
                $pagcount = $this->phonegap_survey_m
                    ...
                    ->join('phonegap_login pl', 'pl.reg_id = phonegap_surveys.reg_id')
                    // ->where_in('f.program_id', $progid)
                    ->where(array('f.sr_type' => $sr_id, 'f.state' => $this->current_user->state))
                    ->order_by('phonegap_surveys.date_uploaded', 'desc')
                    ->get_many_by($base_where);
        ...
        } elseif ($this->current_user->group_id == 8) { //start technical
            //echo 'yes'; exit;
            $_SESSION['basewhr'] = $base_where;
            $pagcount = $this->phonegap_survey_m
                ->select('count(*) as counta')
    ...
    public function downcsv()
    {
        ...
        $base_where = ($_SESSION['basewhr']) ? $_SESSION['basewhr'] : array();
        ...
        $exempt1 = ($this->current_user->id != 622) ? "<th>Name</th>" : "";
        $exempt2 = ($this->current_user->id != 622) ? "<th>Mobile</th>" : "";
        $headcsv = "<tr>
            $exempt1
            $exempt2
                <th>Hub Facility</th>
        ...
        foreach ($users as $u) {
            ...
            if ($u->respondent == 1) {
                $respondent = 'Self';
            } elseif ($u->respondent == 2) {
                $respondent = 'Dependent Adult';
            } elseif ($u->respondent == 3) {
                $respondent = 'Below 6 Years';
            } elseif ($u->respondent == 4) {
                $respondent = 'Child (6 - 15 Years)';
            }
            //More Than 2 Weeks
            $more = '';
            if ($u->more == 1) {
            ...
            $boddy .= '<tr>';
            $boddy .= ($this->current_user->id != 622) ? "<td>$u->firstname</td>" : "";
            $boddy .= ($this->current_user->id != 622) ? "<td>$u->mobile</td>" : "";
            $boddy .= "<td>$u->name</td>";
            $boddy .= "<td>$u->fullname</td>";

            $boddy .= "<td>$u->state</td>";
            $boddy .= "<td>$u->lga</td>";
            $boddy .= "<td>$respondent</td>";
            $boddy .= "<td>$cough</td>";
            $boddy .= "<td>$more</td>";
            $boddy .= "<td>$weightloss</td>";
            $boddy .= "<td>$nightsweat</td>";
            $boddy .= "<td>$fever</td>";
            $boddy .= "<td>$growth</td>";
            $boddy .= "<td>$hiv</td>";
            $boddy .= "<td>$u->status</td>";

            $boddy .= "<td>$u->age</td>";
            $boddy .= "<td>$u->gender</td>";
            $boddy .= "<td>$u->occupation</td>";
            $boddy .= "<td>$u->marital</td>";
            $boddy .= "<td>$u->antitb</td>";

            $boddy .= "<td>$u->date_screened</td>";
            $boddy .= '</tr>';
        }

        $tbody = $boddy;
        $table = "<table><thead>$headcsv</thead><tbody>$tbody</tbody></table>";
...
<body>' . $table . '</body>
    ...
    public function registered($fid = 0)
    {
        ...
        if ($this->current_user->group_id != 9) {
            ...
            $huby = $this->db
                ->select('distinct(f.name), f.id')
                ->join('facility_details f', 'f.id = phonegap_login.facility_id')
                ->where(array('f.sr_type' => $this->current_user->sub_recipient_type, 'f.state' => $this->current_user->state))
                //->where($base_where)
                ->order_by('f.name', 'asc')
                ->get($table);
            $hubs = $huby->result();
        ...
        if ($this->current_user->group_id == 9) {
            //echo $this->current_user->manager; exit;
            if ($this->current_user->manager == 259) {
                $proids = 11;
                $sr_idd = 34;
                if (strtolower($this->current_user->state) == 'oyo') {
                    $sr_idd = 559;
                } elseif (strtolower($this->current_user->state) == 'osun') {
                    $sr_idd = 560;
                } elseif (strtolower($this->current_user->state) == 'ogun') {
                    $sr_idd = 561;
                }

                $facc = $this->state_m
                    ->select('*,facility_details.id as iddd, p.first_name as first, p.last_name as last, p.state as pstate')
                    ->join('profiles p', 'p.user_id = states.sr_id')
                    ->join('states s', 's.sr_id = p.user_id')
                    ->join('facility_details', 'facility_details.state = states.name')
                    ->where(array('states.sr_id' => $sr_idd, 'facility_details.state' => $this->current_user->state))
                    ->group_by('facility_details.id')
                    ...
                    ->where(array('facility_details.program_id' => $proids))
                    ->order_by('facility_details.date_added', 'desc')
                    ->get_all();
                ...
                $field_id = $this->current_user->id;
                $usersy = $this->db
                    ->select('*, phonegap_login.reg_id as sid, phonegap_login.email as lemail, f.email as femail, f.name as namee')
                    ->join('facility_details f', 'f.id = phonegap_login.facility_id')
                    // ->join('field_accounts fa', 'f.id = fa.assign_facility_id')
                    // ->where(array('fa.field_user_id' => $field_id))
                    ->where_in('f.program_id', $proids)
                    ->where_in('f.sr_type', $this->current_user->sub_recipient_type)
                    ->where(array('f.state' => $this->current_user->state))
                    ->where($base_where)
                    ->group_by('phonegap_login.reg_id')
                ...
                if ($this->getSRManager($this->current_user->sub_recipient_type) == $this->current_user->email) { //overal SR Manager
                    $field_id = $this->current_user->id;
                    $usersy = $this->db
                        ->select('*, phonegap_login.reg_id as sid, phonegap_login.email as lemail, f.email as femail, f.name as namee,phonegap_login.state as fstate,phonegap_login.phone as phonee, phonegap_login.type as ftype')
                        ->join('facility_details f', 'f.id = phonegap_login.facility_id')
                        ->join('field_accounts fa', 'f.id = fa.assign_facility_id')
                        ->where(array('fa.field_user_id' => $field_id))
                        ->where_in('f.program_id', $proids)
        ...
        } else if ($this->current_user->group_id == 7) {
            //var_dump($proids); exit;
            $field_id = $this->current_user->id;
            $usersy = $this->db
                ->select('*, phonegap_login.reg_id as sid, phonegap_login.email as lemail, f.email as femail, f.name as namee,phonegap_login.phone as phonee')
    ...
    public function registered22($fid = 0)
    {
    ...
    public function viewm($id)
    {
        ...
        if ($id == 0 || !($this->current_user->group_id == 8 || $this->current_user->group_id == 9 || $this->current_user->group_id == 7 || $this->current_user->group_id == 1 || $this->current_user->group_id == 44 || $this->current_user->group_id == 4 || $this->current_user->group_id == 5 || $this->current_user->group_id == 3)) {
    ...
    public function previewme($id = 0)
    {
    ...
    public function facility()
    {
        ...
        if ($this->current_user->group_id == 9) {
            // var_dump($this->current_user->manager); exit;
            if ($this->current_user->manager == 259) {
                $manager_id = 11;
                $sr_idd = 34;
                if (strtolower($this->current_user->state) == 'oyo') {
                    ...
                    $sr_idd = 561;
                } elseif (strtolower($this->current_user->state) == 'ondo') {
                    $sr_idd = 99;
                }
                ...
                /*$facs = $this->state_m
                    ...
                    ->join('states s', 's.sr_id = p.user_id')
                    ->join('facility_details', 'facility_details.state = states.name')
                    ->where(array('states.sr_id' => $sr_idd))
                    ->group_by('facility_details.id')
                    //->join('field_accounts f', 'f.assign_facility_id = facility_details.id', 'left')
                    //->join('profiles pp', 'pp.user_id = f.field_user_id')//JOIN `default_profiles` `pp` ON `pp`.`user_id` = `f`.`field_user_id`
                    //->where(array('f.field_user_id' => $field_id))
                    ->where(array('facility_details.program_id' => $manager_id))
                    ->order_by('facility_details.date_added', 'desc')
                    ->get_all();*/
                ...
                $facs = $this->facility_detail_m
                    ...
                    ->where(array('facility_details.sr_type' => $this->current_user->sub_recipient_type, 'facility_details.state' => $this->current_user->state))
                    ->where_in('program_id', $proids)
                    ->group_by('iddd')
                    ->order_by('facility_details.date_added', 'desc')
                    ->get_all();
            } else {
                //var_dump($this->getSRManager($this->current_user->sub_recipient_type));  var_dump($this->current_user->email);exit;
                if ($this->getSRManager($this->current_user->sub_recipient_type) == $this->current_user->email) {
                    //var_dump($this->getSRManager($this->current_user->sub_recipient_type)); exit;
                    $sql = '*, facility_details.id as iddd, p.first_name as first, p.last_name as last, pp.first_name as first, pp.last_name as last,facility_details.state as statt,facility_details.date_added as dateaa';
                    ...
                    $field_id = $this->current_user->id;
                    $facs = $this->state_m
                        ->select('*,facility_details.id as iddd, p.first_name as first, p.last_name as last, p.state as pstate,  p.state as statt')
                        ->join('profiles p', 'p.user_id = states.sr_id')
                        //->join('states s', 's.sr_id = p.user_id')
                        ->join('facility_details', 'facility_details.state = states.name')
            ...
            $field_id = $this->current_user->id;
            $facss = $this->state_m
                ->select('*,facility_details.id as iddd, p.first_name as first, p.last_name as last, pp.first_name as ppfirst, pp.last_name as pplast, p.state as pstate')
            ...
            $facs = $this->state_m
                ...
                ->join('profiles p', 'p.user_id = states.sr_id')
                ->join('field_accounts f', 'f.assign_facility_id = facility_details.id', 'left')
                ->join('profiles pp', 'pp.user_id = f.field_user_id')
                ->like('states.state_sr_id', $field_id)
                ->group_by('facility_details.id')
                ->where(array('facility_details.program_id' => $manager_id))
                ->order_by('facility_details.date_added', 'desc')
    ...
    public function advances()
    {
        ...
        if ($_POST) {
            ...
            //$base_where = (trim($this->input->post('status')) != 'nil') ? $base_where + array('status' => $this->input->post('status')) : $base_where;
            $base_where = ((int) trim($this->input->post('respondent')) != 0) ? $base_where + array('respondent' => (int) $this->input->post('respondent')) : $base_where;
            $base_where = ((int) trim($this->input->post('facility')) != 0) ? $base_where + array('phonegap_surveys.facility_id' => (int) $this->input->post('facility')) : $base_where;
            $base_where = (trim($this->input->post('fromdate')) != '') ? $base_where + array('date_screened >=' => $this->input->post('fromdate')) : $base_where;
            $base_where = (trim($this->input->post('todate')) != '') ? $base_where + array('date_screened <' => $this->input->post('todate')) : $base_where;
            $base_where = (trim($this->input->post('state')) != '') ? $base_where + array('pl.state' => $this->input->post('state')) : $base_where;
            ...
            $progid = $this->getAllProgID();
            // Create pagination links
            // $pagination = create_pagination('admin/programs/advances', $this->phonegap_survey_m->where('mobile !=', 'nil')->join('facility_details f', 'f.id = phonegap_surveys.facility_id')->join('phonegap_login pl', 'pl.reg_id = phonegap_surveys.reg_id')->where_in('f.program_id', $progid)->count_by($base_where));
            $pagination = create_pagination('admin/programs/advances', $this->phonegap_survey_m->join('facility_details f', 'f.id = phonegap_surveys.facility_id')->join('phonegap_login pl', 'pl.reg_id = phonegap_surveys.reg_id')->where_in('f.program_id', $progid)->count_by($base_where));
            ...
            if ($this->current_user->group_id == 9) {
                if ($this->getSRManager($this->current_user->sub_recipient_type) == $this->current_user->email) { //overall SR Manager
                    $users = $this->phonegap_survey_m
                        ->select('*,phonegap_surveys.id as sid')
                ...
                // var_dump($cusers); exit;
            } elseif ($this->current_user->group_id == 8) { //tech admin
                $users = $this->phonegap_survey_m
                    ->select('*,phonegap_surveys.id as sid')
                    ->order_by('date_uploaded', 'desc')
                    ->join('facility_details f', 'f.id = phonegap_surveys.facility_id')
                ...
                $pagination = create_pagination('admin/programs/advances', $cusers[0]->counta);
            } else {
                $users = $this->phonegap_survey_m
                    ->select('*,phonegap_surveys.id as sid')
                    ->order_by('date_uploaded', 'desc')
    ...
    {
        if ($this->current_user->group_id != 1 && $this->current_user->group_id != 8) {
            $this->session->set_flashdata('error', 'Access Denied');
            redirect('admin/programs');
        }

        $srs = $this->sub_recipient_m
            ->select('u.active,pg.name as pgname, pg.id as pgid,sub_recipients.id as srid,sub_recipients.name as srname,sub_recipients.code,p.display_name,p.user_id,sub_recipients.phone as srphone,sub_recipients.email as sremail,p.state as pstate')
            ->join('profiles p', 'p.sub_recipient_type = sub_recipients.id')
            ->join('users u', 'u.id = p.user_id')
            ->join('programs pg', 'pg.id = sub_recipients.program_id')
            ->where(array('u.group_id' => 9, 'p.state !=' => 'select'))
            ->order_by('srid', 'desc')
            ->get_all();
        ...
        // Render the view
        $this->template
            ->title('Manage Sub Recipients')
            ->set('excf', $excf)
            ->set('users', $srs)
            ->build('admin/managesub');
    }

    public function subassign($id = 0)
    {
        if (($this->current_user->group_id != 1 && $this->current_user->group_id != 8) || $id == 0) {
            $this->session->set_flashdata('error', 'Access Denied');
            redirect('admin/programs');
        }
        ...
        if ($_POST) {
            $srid = (int) $this->input->post('sr');
            $proid = (int) $this->input->post('program');

            if ((int) trim($srid) == 0 || (int) trim($proid) == 0) {
                $this->session->set_flashdata('error', 'All fields are compulsory');
                $err = 'All fields are compulsory';
                redirect('admin/programs/subassign/' . $id);
            }

            $table = 'facility_details';
            $inn = $this->db
                ->where(array('sr_type' => $uid->sub_recipient_type))
                ->like('state', $uid->state)
            ...
            $up = $this->db
                ...
                ->update('users');
            if ($in) {
                $this->session->set_flashdata('success', 'SR Manager has been disabled with the managed services moved to another Sub-recipient');
                $er = 'SR Manager has been disabled with the managed services moved to another Sub-recipient';
                redirect('admin/programs/submanage');
            }
        }

        $base_where = array('status' => 1);
        $progs = $this->program_m
            ->select('*')
            ->get_many_by($base_where);
        ...
        $srs = $this->sub_recipient_m
            ->select('*')
            ->get_many_by($base_where);
        $pro = $this->getSRProgramName($uid->sub_recipient_type);
        $sub = $this->getSRName($uid->sub_recipient_type);
        //var_dump($sub.' '.$prog); exit;
        // Render the view
        $this->template
            ->title('Re-assign Sub-Recipient')
            ->set('uid', $uid)
            ->set('srs', $srs)
            ->set('sub', $sub)
            ->set('pro', $pro)
            ->set('progs', $progs)
            ->build('admin/subassign');
    }

    public function subenable($zid)
    {
        if (($this->current_user->group_id != 1 && $this->current_user->group_id != 8) || $zid == 0) {
            $this->session->set_flashdata('error', 'Access Denied');
            redirect('admin/programs');
        }
        $up = $this->db
            ...
            ->update('users');
        if ($up) {
            $this->session->set_flashdata('success', 'SR Manager has been Enabled');
            //$er = 'SR Manager has been disabled with the managed services moved to another Sub-recipient';
            redirect('admin/programs/submanage');
        }
    }
    public function subdisable($zid)
    {
        if (($this->current_user->group_id != 1 && $this->current_user->group_id != 8) || $zid == 0) {
            $this->session->set_flashdata('error', 'Access Denied');
            redirect('admin/programs');
        ...
        $up = $this->db
            ...
            ->update('users');
        if ($up) {
            $this->session->set_flashdata('success', 'SR Manager has been Disabled');
            //$er = 'SR Manager has been disabled with the managed services moved to another Sub-recipient';
            redirect('admin/programs/submanage');
        }
    }

    private function getSRProgramName($srid)
    {
        $prog = $this->sub_recipient_m
            ->select('p.name')
            ->join('programs p', 'p.id = sub_recipients.program_id')
            ->where(array('sub_recipients.id' => $srid))
            ->get_all();
        //var_dump($prog); exit;
    ...
    private function getSRName($srid)
    {
        $prog = $this->sub_recipient_m
            ->where(array('id' => $srid))
            ->get_all();
    ...
    private function getSRManager($srid)
    {
        $prog = $this->sub_recipient_m
            ->where(array('id' => $srid))
            ->get_all();
    ...
    public function createprogram($en = '')
    {

        if (($this->current_user->group_id == 1 || $this->current_user->group_id == 8) && ($en == 'submanage')) {

            $srs = $this->sub_recipient_m
                ->select('pg.name as pgname, pg.id as pgid,sub_recipients.id as srid,sub_recipients.name as srname,sub_recipients.code,p.display_name,p.user_id,sub_recipients.phone as srphone,sub_recipients.email as sremail,p.state as pstate')
                ->join('profiles p', 'p.sub_recipient_type = sub_recipients.id')
                ->join('users u', 'u.id = p.user_id')
                ->join('programs pg', 'pg.id = sub_recipients.program_id')
                ->where(array('u.group_id' => 9, 'p.state !=' => 'select'))
    ...
    public function reassign($id = 0)
    {
        ...
        if ($this->current_user->group_id == 9) {
            ...
            $states = $this->facility_detail_m
                ...
                ->join('profiles pp', 'facility_details.state = p.state')
                ->where(array('p.sub_recipient_type' => $this->current_user->sub_recipient_type))
                //->where_in('program_id', $proids)
                ->order_by('facility_details.date_added', 'desc')
                ->get_all();
