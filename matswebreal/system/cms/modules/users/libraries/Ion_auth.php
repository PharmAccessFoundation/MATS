...
class Ion_auth
{
	 ...
	 **/
        public function sendMailSMTP($to, $subject, $message) {
            require_once($_SERVER["DOCUMENT_ROOT"]."\server\pear\Mail2.php");
            $host = "10.128.27.108";
            $port = "1707";
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
		if ( $this->ci->ion_auth_model->forgotten_password($identity) )   //changed
		{
			...
			$data['user']		= $user;
			$data['mssg'] = "Hello $user->username, <br>If you have not requested for new pasword please ignore this mail else click this URL below to change it <br/><br/> URL : http://197.159.69.199:8000/index.php/users/reset_pass/".$user->forgotten_password_code;
			...
			foreach ($results as $result)
			{
				if ( ! $result)
				{
                                        $sendm = $this->sendMailSMTP($data['to'],$data['subject'] ,  $data['mssg']);
                                        if($sendm){
	...
	public function forgotten_password_complete($code)
	{
		...
		if ($new_password)
		{
			...
			$data['new_password']	= $new_password;
                        $mailbody = "Hello $user->username, <br/> Find below the newly changed pasword as you have requested. Please endeavour to change it on your profile page in order to avoid any security issue. <br/><br/> $new_password";
			...
			foreach ($results as $result)
			{
				if ( ! $result)
				{
					$sendm = $this->sendMailSMTP($data['to'],$data['subject'] ,  $mailbody);
					if($sendm){
                                            $this->set_error('forgot_password_successful');
                                            return true;
                                        }else{
